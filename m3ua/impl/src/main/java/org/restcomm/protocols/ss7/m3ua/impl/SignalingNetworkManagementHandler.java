...
public class SignalingNetworkManagementHandler extends MessageHandler {
			...
			if (rcObj == null) {
				...
				if (aspImpl == null) {
					...
					logger.error(String.format(
							"Rx : DUNA=%s with null RC for Aspfactory=%s. But no ASP configured for null RC. Sending back Error",
				...
				for (int count = 0; count < rcs.length; count++) {
					...
					if (aspImpl == null) {
						...
						logger.error(String.format(
								"Rx : DUNA=%s with RC=%d for Aspfactory=%s. But no ASP configured for this RC. Sending back Error",
		...
		if (aspFactoryImpl.getFunctionality() == Functionality.AS) {
			...
			if (rcObj == null) {
				...
				if (aspImpl == null) {
					...
					sendError(rcObj, errorCodeObj);
					logger.error(String.format(
							"Rx : DAVA=%s with null RC for Aspfactory=%s. But no ASP configured for null RC. Sending back Error",
				...
				for (int count = 0; count < rcs.length; count++) {
					...
					if (aspImpl == null) {
						...
						sendError(rcObjTemp, errorCodeObj);
						logger.error(String.format(
								"Rx : DAVA=%s with RC=%d for Aspfactory=%s. But no ASP configured for this RC. Sending back Error",
			...
			logger.error(String.format(
					"Rx : DAVA =%s But AppServer Functionality is not As. Sending back ErrorCode.Unexpected_Message",
					dava));
	...
	public void handleDestinationStateAudit(DestinationStateAudit daud) {
		...
		if (aspFactoryImpl.getFunctionality() == Functionality.SGW)
			...
			logger.error(String.format(
					"Rx : DAUD =%s But AppServer Functionality is not SGW. Sending back ErrorCode.Unexpected_Message",
					daud));
		...
		if (aspFactoryImpl.getFunctionality() == Functionality.AS
			...
			if (rcObj == null) {
				...
				if (aspImpl == null) {
					...
					sendError(rcObj, errorCodeObj);
					logger.error(String.format(
							"Rx : SCON=%s with null RC for Aspfactory=%s. But no ASP configured for null RC. Sending back Error",
				...
				for (int count = 0; count < rcs.length; count++) {
					...
					if (aspImpl == null) {
						...
						sendError(rcObjTemp, errorCodeObj);
						logger.error(String.format(
								"Rx : SCON=%s with RC=%d for Aspfactory=%s. But no ASP configured for this RC. Sending back Error",
		...
		if (aspFactoryImpl.getFunctionality() == Functionality.AS) {
			...
			if (rcObj == null) {
				...
				if (aspImpl == null) {
					...
					sendError(rcObj, errorCodeObj);
					logger.error(String.format(
							"Rx : DUPU=%s with null RC for Aspfactory=%s. But no ASP configured for null RC. Sending back Error",
				...
				for (int count = 0; count < rcs.length; count++) {
					...
					if (aspImpl == null) {
						...
						sendError(rcObjTemp, errorCodeObj);
						logger.error(String.format(
								"Rx : DUPU=%s with RC=%d for Aspfactory=%s. But no ASP configured for this RC. Sending back Error",
			...
			logger.error(String.format(
					"Rx : DUPU =%s But AppServer Functionality is not AS. Sending back ErrorCode.Unexpected_Message",
					dupu));
