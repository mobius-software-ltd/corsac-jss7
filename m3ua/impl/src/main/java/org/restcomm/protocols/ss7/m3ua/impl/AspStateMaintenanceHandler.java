...
public class AspStateMaintenanceHandler extends MessageHandler {
	...
	protected void handleAspUp(ASPUp aspUp) {
		...
		if (aspFactoryImpl.getFunctionality() == Functionality.SGW
				|| (aspFactoryImpl.getFunctionality() == Functionality.AS
				...
				|| (aspFactoryImpl.getFunctionality() == Functionality.IPSP
						...
						&& aspFactoryImpl.getIpspType() == IPSPType.SERVER)) {

			ASPUpAck aspUpAck = (ASPUpAck) this.aspFactoryImpl.messageFactory
					.createMessage(MessageClass.ASP_STATE_MAINTENANCE, MessageType.ASP_UP_ACK);
	...
	protected void handleAspUpAck(ASPUpAck aspUpAck) {
		...
		if (aspFactoryImpl.getFunctionality() == Functionality.AS
			...
			while (iterator.hasNext()) {
				...
				else // Transition to ACTIVE_SENT
					try {
						...
						if (aspFactoryImpl.getFunctionality() == Functionality.IPSP) {
							...
							if (asPeerFSM == null) {
								logger.error(String.format(
										...
										this.aspFactoryImpl.getName(), asImpl));
								return;
							}
	...
	protected void handleAspDown(ASPDown aspDown) {
		...
		if (aspFactoryImpl.getFunctionality() == Functionality.SGW
			...
			ASPDownAck aspDwnAck = (ASPDownAck) this.aspFactoryImpl.messageFactory
	...
	protected void handleAspDownAck(ASPDownAck aspUpAck) {
		...
		if (!this.aspFactoryImpl.started) {
			...
			if (!stopAssociation)
				logger.error(String.format(
						"Ungracefully stopping the underlying Association=%s for AspFactory=%s. Atleast one of the ASP's is Double Exchange and Peer ASP state is still not down",
						this.aspFactoryImpl.association.getName(), this.aspFactoryImpl.getName()));

			try {
				if (this.aspFactoryImpl.aspFactoryStopTimer != null)
					this.aspFactoryImpl.aspFactoryStopTimer.stop();
				this.aspFactoryImpl.transportManagement.stopAssociation(this.aspFactoryImpl.association.getName());
			} catch (Exception e) {
				logger.error(String.format("Exception while starting the Association=%s",
						this.aspFactoryImpl.association.getName()), e);
			}
		} else
	...
	public void handleHeartbeat(Heartbeat hrtBeat) {
		HeartbeatAck hrtBeatAck = (HeartbeatAck) this.aspFactoryImpl.messageFactory
				.createMessage(MessageClass.ASP_STATE_MAINTENANCE, MessageType.HEARTBEAT_ACK);
