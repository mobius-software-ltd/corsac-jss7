...
public class USSDStringImpl extends ASNOctetString implements USSDString {
	...
	public static ByteBuf translate(String ussdString, CBSDataCodingScheme dataCodingScheme, Charset gsm8Charset)
			throws MAPException {
		if (ussdString == null)
			ussdString = "";
		if (dataCodingScheme == null)
		...
		if (dataCodingScheme.getIsCompressed())
			...
			case UCS2:
				if (dataCodingScheme.getDataCodingGroup() == CBSDataCodingGroup.GeneralWithLanguageIndication) {
					...
					encoder = cSet.newEncoder();
					encoder.setGSMCharsetEncodingData(
							new GSMCharsetEncodingData(Gsm7EncodingStyle.bit7_ussd_style, null));
					bb = null;
					try {
		...
		if (dataCodingScheme.getIsCompressed())
			...
			case GSM7:
				...
				decoder.setGSMCharsetDecodingData(
						new GSMCharsetDecodingData(Gsm7EncodingStyle.bit7_ussd_style, Integer.MAX_VALUE, 0));
				String bf = null;
				try {
