...
public class LSAInformationImpl implements LSAInformation {
	private ASNNull completeDataListIncluded;
    
    @ASNProperty(asnClass=ASNClass.CONTEXT_SPECIFIC,tag=1,constructed=false,index=-1)
    private ASNLSAOnlyAccessIndicator lsaOnlyAccessIndicator;    
    
    @ASNProperty(asnClass=ASNClass.CONTEXT_SPECIFIC,tag=2,constructed=true,index=-1)
    private LSADataListWrapperImpl lsaDataList;
    ...
    public List<LSAData> getLSADataList() {
    	if(this.lsaDataList==null)
    		return null;
    	
        return this.lsaDataList.getLSAData();
    }
    ...
    public String toString() {
        ...
        if (this.lsaOnlyAccessIndicator != null) {
            ...
            sb.append(this.lsaOnlyAccessIndicator.getType());
            sb.append(", ");
        }

        if (this.lsaDataList != null && this.lsaDataList.getLSAData()!=null) {
            sb.append("lsaDataList=[");
            boolean firstItem = true;
            for (LSAData be : this.lsaDataList.getLSAData()) {
                if (firstItem)
                    firstItem = false;
                else
                    sb.append(", ");
