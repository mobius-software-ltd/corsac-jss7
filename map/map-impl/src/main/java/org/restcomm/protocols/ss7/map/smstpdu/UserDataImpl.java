...
public class UserDataImpl implements UserData {
	...
	public UserDataImpl(ByteBuf messageWithSkipBits, DataCodingScheme dataCodingScheme,
	...
	private void encodeMessage() {
		...
		if (!this.dataCodingScheme.getIsCompressed())
			...
			case GSM7:
				...
				GSMCharsetEncoder encoder = cSet.newEncoder();
				encoder.setGSMCharsetEncodingData(
						new GSMCharsetEncodingData(Gsm7EncodingStyle.bit7_sms_style, udhLength));
				try {
	...
	private void decodeMessage() {
		...
		case GSM7:
			GSMCharset cSet = obtainGsmCharacterSet(this.decodedUserDataHeader);
			GSMCharsetDecoder decoder = cSet.newDecoder();

			int septetOffset = 0;
			int bitPos = 0;
			if (decodedUserDataHeader != null) {
				bitPos = decodedUserDataHeader.getLength() * 8;
				bitPos = 7 - (bitPos - (bitPos / 7) * 7);
			}

			decoder.setGSMCharsetDecodingData(new GSMCharsetDecodingData(Gsm7EncodingStyle.bit7_sms_style,
					this.userDataLength - septetOffset, bitPos));
			String bf = null;
			try {
				bf = decoder.decode(this.messageWithSkipBits.slice());
	...
	public void encode() throws MAPException {
		...
		if (buf2 != null) {
			ByteBuf combined = Unpooled.buffer(buf2.readableBytes() + messageWithSkipBits.readableBytes());
			combined.writeBytes(buf2);
			combined.writeBytes(messageWithSkipBits);

			this.encodedData = combined;
		} else
	...
	private static GSMCharset obtainGsmCharacterSet(UserDataHeader udh) {
		...
		if (nationalLanguageLockingShift != null || nationalLanguageSingleShift != null)
			cSet = new GSMCharset(nationalLanguageLockingShiftIdentifier, nationalLanguageSingleShiftIdentifier);
		return cSet;
	}
