...
public class MGCSIImpl implements MGCSI {
    ...
    public MGCSIImpl(List<MMCode> mobilityTriggers, int serviceKey, ISDNAddressString gsmSCFAddress,
	...
	public void validateElement() throws ASNParsingComponentException {
		if(mobilityTriggers==null || mobilityTriggers.getMMCode()==null || mobilityTriggers.getMMCode().size()==0)
			throw new ASNParsingComponentException("mobility triggers should be set for mgcs", ASNParsingComponentExceptionReason.MistypedParameter);

		if(mobilityTriggers.getMMCode().size()>10)
			throw new ASNParsingComponentException("mobility triggers should be set for mgcs", ASNParsingComponentExceptionReason.MistypedParameter);

		if(serviceKey==null)
			throw new ASNParsingComponentException("service key should be set for mgcs", ASNParsingComponentExceptionReason.MistypedParameter);

		if(gsmSCFAddress==null)
			throw new ASNParsingComponentException("gsm scf address should be set for mgcs", ASNParsingComponentExceptionReason.MistypedParameter);
	}
}
