...
public interface MAPDialog extends Serializable {
	...
	/**
	 * Return received OrigReference from MAPOpenInfo or null if no OrigReference
	 * has been received
	 *
	 * @return
	 */
	...
	/**
	 * Return received DestReference from MAPOpenInfo or null if no OrigReference
	 * has been received
	 *
	 * @return
	 */
	...
	/**
	 * Option responsible for presence of the protocol version in this dialogue
	 * portion.
	 *
	 * @return boolean true if protocol version must be omitted, false when it
	 *         should be included and null if not defined at the dialog level and
	 *         global option should be used.
	 */
	Boolean isDoNotSendProtcolVersion();

	/**
	 * Modifies option responsible for presence of the protocol version in this
	 * dialogue portion.
	 *
	 * @param doNotSendProtocolVersion boolean true if protocol version must be
	 *                                 omitted, false when it should be included and
	 *                                 null if not defined at the dialog level and
	 *                                 global option should be used.
	 */
	...
	/**
	 * Set ExtentionContainer that will be send in 1) T-BEGIN 2) T-CONTINUE or T-END
	 * if it is response to the T-BEGIN 3) T-ABORT If no Dialogue control APDU is
	 * sending - ExtentionContainer will also not be sent
	 */
	 ...
	 * send() is called to explicitly request the transfer of the MAP protocol data
	 * units to the peer entities.
	 */
	void send(TaskCallback<Exception> callback);

	/**
	 * This is equivalent of MAP User issuing the MAP_CLOSE Service Request. This
	 ...
	 * <br/>
	 *
	 * If prearrangedEnd is false, all the Service Primitive added to MAPDialog and
	 * not sent yet, will be sent to peer.
	 *
	 * <br/>
	 *
	 * If prearrangedEnd is true, all the Service Primitive added to MAPDialog and
	 * not sent yet, will not be sent to peer.
	 *
	 ...
	 * @param userReason
	 */
	void abort(MAPUserAbortChoice mapUserAbortChoice, TaskCallback<Exception> callback);

	/**
	 * Send T_U_ABORT with MAP-RefuseInfo
	 */
	void refuse(Reason reason, TaskCallback<Exception> callback);

	/**
	 * If a MAP user will not answer to an incoming Invoke with Response, Error or
	 * Reject components it should invoke this method to remove the incoming Invoke
	 * from a pending incoming Invokes list
	 *
	 * This do not affect class 1 messages hence most of MAP operations do not need
	 * it
	 *
	 * @param invokeId
	 */
	...
	/**
	 * Store in the MAPDialog a user-defined object to save relating to the Dialog
	 * information
	 *
	 * @param userObject
	 */
	...
	/**
	 * Return the maximum MAP message length (in bytes) that are allowed for this
	 * dialog
	 *
	 * @return
	 */
