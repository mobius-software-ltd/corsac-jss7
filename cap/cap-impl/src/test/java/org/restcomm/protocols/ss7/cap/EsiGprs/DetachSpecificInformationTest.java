...
public class DetachSpecificInformationTest {
    ...
    public void testDecode() throws Exception {
    	...
    	parser.replaceClass(DetachSpecificInformationImpl.class);
    	
    	byte[] rawData = this.getData();
        ASNDecodeResult result=parser.decode(Unpooled.wrappedBuffer(rawData));

        assertFalse(result.getHadErrors());
        assertTrue(result.getResult() instanceof DetachSpecificInformationImpl);
        
        DetachSpecificInformationImpl prim = (DetachSpecificInformationImpl)result.getResult(); 
