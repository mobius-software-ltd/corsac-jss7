...
public abstract class SccpDataNoticeTemplateMessageImpl extends SccpSegmentableMessageImpl {
	...
	protected abstract void getSecondParamaterData(ByteBuf data, boolean removeSPC,
			SccpProtocolVersion sccpProtocolVersion) throws ParseException;

	protected abstract void setSecondParamaterData(ByteBuf data, SccpProtocolVersion sccpProtocolVersion)
			throws ParseException;

	@Override
	public void decode(ByteBuf buffer, ParameterFactory factory, SccpProtocolVersion sccpProtocolVersion)
			throws ParseException {
		try {
			switch (this.type) {
			...
			case SccpMessage.MESSAGE_TYPE_UDTS: {
				...
				super.calledParty = createAddress(buffer.slice(buffer.readerIndex(), len), factory,
						sccpProtocolVersion);
				...
				super.callingParty = createAddress(buffer.slice(buffer.readerIndex(), len), factory,
						sccpProtocolVersion);
				buffer.resetReaderIndex();
			...
			case SccpMessage.MESSAGE_TYPE_XUDTS: {
				...
				this.hopCounter = new HopCounterImpl(buffer.readByte());
	...
	private void decodeOptional(int code, ByteBuf buffer, final SccpProtocolVersion sccpProtocolVersion)
			throws ParseException {
	...
	@Override
	public EncodingResultData encode(SccpStackImpl sccpStackImpl, LongMessageRuleType longMessageRuleType,
		...
		((SccpAddressImpl) super.calledParty).encode(cdp, sccpStackImpl.isRemoveSpc(),
				sccpStackImpl.getSccpProtocolVersion());
		((SccpAddressImpl) super.callingParty).encode(cnp, sccpStackImpl.isRemoveSpc(),
				sccpStackImpl.getSccpProtocolVersion());

		if (longMessageRuleType == null)
			longMessageRuleType = LongMessageRuleType.LONG_MESSAGE_FORBBIDEN;
		if (this.isMtpOriginated && this.type == SccpMessage.MESSAGE_TYPE_UDT
		...
		if (useShortMessage) {
			...
			if (data.readableBytes() > availLen) { // message is too long to encode UDT
				if (logger.isWarnEnabled())
					logger.warn(String.format(
							"Failure when sending a UDT message: message is too long. Availabile Length:" + availLen
									+ ",Message Length:" + data.readableBytes() + ",SccpMessageSegment=%s",
							this));
				return new EncodingResultData(EncodingResult.ReturnFailure, null, null,
						ReturnCauseValue.SEG_NOT_SUPPORTED);
			}
			...
			if (data.readableBytes() <= availLenX && data.readableBytes() <= sccpStackImpl.getZMarginXudtMessage()) {
				...
				// we have 4 pointers, cdp,cnp,data and optionalm, cdp starts after 4 octests
				// than
				int len = 4;
				out.writeByte(len);
				...
				if (this.isMtpOriginated) {
					if (this.segmentation == null) {
						...
						if (logger.isWarnEnabled())
							logger.warn(String.format(
									"Failure when segmenting a message: message is not locally originated but \"segmentation\" field is absent. SccpMessageSegment=%s",
									this));
						return new EncodingResultData(EncodingResult.ReturnFailure, null, null,
								ReturnCauseValue.SEG_FAILURE);
					}

					this.segmentation = new SegmentationImpl(true, this.segmentation.isClass1Selected(),
							(byte) segmCount, this.segmentation.getSegmentationLocalRef());
				} else
			...
			// we have 4 pointers, cdp,cnp,data and optionalm, cdp starts after 8 octests
			// than
			int len = 7;
	...
	protected SccpAddress createAddress(ByteBuf buffer, ParameterFactory factory,
			SccpProtocolVersion sccpProtocolVersion) throws ParseException {
