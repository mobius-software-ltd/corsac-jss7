...
public class RouteSelectFailureRequestImpl extends CircuitSwitchedCallMessageImpl implements RouteSelectFailureRequest {
	...
	@ASNProperty(asnClass = ASNClass.CONTEXT_SPECIFIC,tag = 3,constructed = false,index = -1,defaultImplementation = CallingPartySubaddressImpl.class)
	...
	@ASNProperty(asnClass = ASNClass.CONTEXT_SPECIFIC,tag = 6,constructed = false,index = -1, defaultImplementation = CauseIsupImpl.class)
	private CauseIsup failureCause;
	
	@ASNProperty(asnClass = ASNClass.CONTEXT_SPECIFIC,tag = 7,constructed = false,index = -1, defaultImplementation = OriginalCalledNumberIsupImpl.class)
	private OriginalCalledNumberIsup originalCalledPartyID;
	
	@ASNProperty(asnClass = ASNClass.CONTEXT_SPECIFIC,tag = 8,constructed = false,index = -1, defaultImplementation = DigitsIsupImpl.class)
	private DigitsIsup prefix;
	
	@ASNProperty(asnClass = ASNClass.CONTEXT_SPECIFIC,tag = 9,constructed = false,index = -1, defaultImplementation = RedirectingPartyIDIsupImpl.class)
	private RedirectingPartyIDIsup redirectingPartyID;
	
	@ASNProperty(asnClass = ASNClass.CONTEXT_SPECIFIC,tag = 10,constructed = false,index = -1, defaultImplementation = RedirectionInformationIsupImpl.class)
	private RedirectionInformationIsup redirectionInformation;
    
	@ASNProperty(asnClass = ASNClass.CONTEXT_SPECIFIC,tag = 11,constructed = true,index = -1, defaultImplementation = RouteListImpl.class)
	private RouteList routeList;
    
	@ASNProperty(asnClass = ASNClass.CONTEXT_SPECIFIC,tag = 12,constructed = false,index = -1, defaultImplementation = LocationNumberIsupImpl.class)
	...
	@ASNProperty(asnClass = ASNClass.CONTEXT_SPECIFIC,tag = 14,constructed = false,index = -1, defaultImplementation = CarrierImpl.class)
	private Carrier carrier;

    public RouteSelectFailureRequestImpl() {
    }

    public RouteSelectFailureRequestImpl(DpSpecificCommonParameters dpSpecificCommonParameters,CalledPartyNumberIsup dialedDigits,
    		CallingPartyBusinessGroupID callingPartyBusinessGroupID,CallingPartySubaddress callingPartySubaddress,FacilityGroup callingFacilityGroup,
    ...
    public INAPMessageType getMessageType() {
        return INAPMessageType.routeSelectFailure_Request;
    }

    @Override
    public int getOperationCode() {
        return INAPOperationCode.routeSelectFailure;
    }
    ...
    public String toString() {
        ...
        sb.append("RouteSelectFailureRequestIndication [");
	...
	public void validateElement() throws ASNParsingComponentException {
		if(dpSpecificCommonParameters==null)
			throw new ASNParsingComponentException("dp specific common parameters should be set for route select failure request", ASNParsingComponentExceptionReason.MistypedRootParameter);
	}
}
